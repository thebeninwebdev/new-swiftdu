...
const pillars = [
  { title: "Support", description: "Get things done easily—food, printing, shopping, errands and more.", cta: "Explore services", href: "/signup", icon: Heart, style: "bg-violet-50 text-violet-600" },
  ...
  { title: "Grow", description: "Learn new skills, build experience and grow through opportunities.", cta: "Start growing", href: "/signup", icon: GraduationCap, style: "bg-blue-50 text-blue-600" },
];
...
const opportunities = [
  { title: "Food pickups", category: "Everyday support", image: supportImage, icon: Coffee, href: "/signup" },
  { title: "Shopping errands", category: "Campus convenience", image: earnImage, icon: ShoppingBag, href: "/signup" },
  { title: "Printing & notes", category: "Academic support", image: learnImage, icon: BookOpen, href: "/signup" },
...
function TrustAndCTA() {
  ...
  return <section className="bg-white px-5 py-24 sm:px-8 sm:py-32 lg:px-10"><div className="mx-auto grid max-w-[1380px] overflow-hidden bg-[#0d1236] text-white lg:grid-cols-[1.05fr_0.95fr]">
    ...
    <div className="relative isolate overflow-hidden bg-gradient-to-br from-violet-700 to-purple-500 p-7 sm:p-10"><div aria-hidden="true" className="absolute -right-20 -top-20 -z-10 h-72 w-72 rounded-full bg-white/15 blur-2xl" /><p className="text-xs font-bold uppercase tracking-[0.18em] text-violet-100">Your place is here</p><h2 className="mt-3 max-w-md text-3xl font-bold tracking-[-0.04em]">Belong. Support. Earn. Learn. Grow.</h2><p className="mt-4 max-w-md text-sm leading-6 text-violet-100">Whether you need help getting things done or have time and skills to give, SwiftDU has a place for you.</p><div className="mt-7 flex flex-col gap-3 sm:flex-row"><Link href="/signup" className="inline-flex items-center justify-center gap-2 rounded-full bg-white px-5 py-3 text-sm font-bold text-violet-700 transition hover:bg-violet-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white">Join the community <ArrowRight className="h-4 w-4" /></Link><Link href="/tasker-signup" className="inline-flex items-center justify-center rounded-full border border-white/40 px-5 py-3 text-sm font-bold text-white transition hover:bg-white/10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white">Become a Swifter</Link></div></div>
  </div></section>;
}
