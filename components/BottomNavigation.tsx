...
export default function TaskerBottomNav() {
  ...
  return <>
    ...
    <div className="pointer-events-none fixed inset-x-0 bottom-0 z-50 lg:hidden" style={{ paddingBottom: 'env(safe-area-inset-bottom)' }}><div className="relative mx-auto h-[96px] w-full max-w-[500px] px-2"><svg viewBox="0 0 500 86" preserveAspectRatio="none" aria-hidden="true" className="pointer-events-none absolute inset-x-2 bottom-1 h-[86px] w-[calc(100%-16px)] drop-shadow-[0_8px_18px_rgba(15,23,42,0.16)]"><motion.path initial={false} animate={{ d: notchPath(index) }} transition={transition} fill="currentColor" className="text-white dark:text-slate-950" /></svg><nav aria-label="Tasker navigation" className="pointer-events-auto absolute inset-x-2 bottom-1 grid h-[86px] grid-cols-5 px-1"><motion.span aria-hidden="true" className={`pointer-events-none absolute top-[-17px] h-[58px] w-[58px] rounded-full bg-gradient-to-br shadow-[0_8px_22px_rgba(124,58,237,0.32)] ${active === 'work' && checkedIn ? 'from-emerald-500 to-emerald-600 shadow-emerald-500/30' : 'from-indigo-600 to-purple-600'}`} animate={{ left: `calc(${(index * 20) + 10}% - 29px)` }} transition={transition} />{navigation.map(item => item.id === 'work' ? <NavButton key={item.id} item={{ ...item, label: work.data?.status === 'busy' ? 'Working' : checkedIn ? 'Check out' : 'Check In' }} active={active === item.id} checkedIn={checkedIn} disabled={work.loading || work.saving || !work.data} onClick={checkIn} /> : item.id === 'more' ? <NavButton key={item.id} item={item} active={active === item.id} onClick={() => setMoreOpen(value => !value)} /> : <NavLink key={item.id} item={item} active={active === item.id} onClick={() => setMoreOpen(false)} />)}</nav></div></div>
  </>
}

function NavLink({ item, active, onClick }: { item: NavItem; active: boolean; onClick: () => void }) { const Icon = item.icon; return <Link href={item.href!} onClick={onClick} aria-current={active ? 'page' : undefined} className="relative flex min-h-[44px] min-w-0 items-center justify-center"><motion.span className="relative z-10 flex flex-col items-center gap-1" animate={{ y: active ? -31 : 5, scale: active ? 1.06 : 1 }} transition={spring}><Icon className={active ? 'h-[22px] w-[22px] text-white' : 'h-[22px] w-[22px] text-slate-500 dark:text-slate-400'} strokeWidth={active ? 2.4 : 2} /><motion.span animate={{ opacity: active ? 0 : 1, y: active ? 6 : 0 }} transition={{ duration: .18 }} className="absolute left-1/2 top-[26px] -translate-x-1/2 whitespace-nowrap text-[10px] font-medium text-slate-500 dark:text-slate-400">{item.label}</motion.span></motion.span></Link> }
function NavButton({ item, active, checkedIn = false, onClick, disabled = false }: { item: NavItem; active: boolean; checkedIn?: boolean; onClick: () => void; disabled?: boolean }) { const Icon = item.icon; const work = item.id === 'work'; return <button type="button" onClick={onClick} disabled={disabled} aria-pressed={work ? checkedIn : undefined} className="relative flex min-h-[44px] min-w-0 items-center justify-center"><motion.span className="relative z-10 flex flex-col items-center gap-1" animate={{ y: active ? -31 : 5, scale: active ? 1.06 : 1 }} transition={spring}><Icon className={active ? 'h-[22px] w-[22px] text-white' : work && checkedIn ? 'h-[22px] w-[22px] text-emerald-600' : 'h-[22px] w-[22px] text-slate-500 dark:text-slate-400'} strokeWidth={active ? 2.4 : 2} /><motion.span animate={{ opacity: active ? 0 : 1, y: active ? 6 : 0 }} transition={{ duration: .18 }} className={work && checkedIn ? 'absolute left-1/2 top-[26px] -translate-x-1/2 whitespace-nowrap text-[10px] font-medium text-emerald-600' : 'absolute left-1/2 top-[26px] -translate-x-1/2 whitespace-nowrap text-[10px] font-medium text-slate-500 dark:text-slate-400'}>{item.label}</motion.span></motion.span></button> }
